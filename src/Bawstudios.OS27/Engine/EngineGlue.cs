...
using Bawstudios.OS27.Model;
using Bawstudios.OS27.Texture;
using Bawstudios.OS27.Steam;

namespace Bawstudios.OS27.Engine;

/// <summary>
/// Emulation module for EngineGlue functions (EngineGlue_*).
/// These functions provide utilities for the engine (KeyValues, StringToken, SearchPath, etc.).
/// </summary>
public static unsafe class EngineGlue
{
    ...
    // Helper to read JSON from a buffer (avoids calling [UnmanagedCallersOnly] directly)
    ...
    public static void Init(void** native)
    {
        ...
        // Indices calculated from Interop.Engine.cs (lines 16070-16085)
        ...
        // Glue_Resources functions (indices from Interop.Engine.cs - to verify)
    ...
    /// <summary>
    /// Converts JSON to KeyValues3 (simplified emulation).
    /// </summary>
    ...
    /// <summary>
    /// Converts KeyValues to JSON (simplified emulation).
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Converts KeyValues3 to JSON (simplified emulation).
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Loads KeyValues3 from a file or string (simplified emulation).
    /// Parses KeyValues text format (starts with '<') and converts it to KeyValues3 structure.
    /// </summary>
    [UnmanagedCallersOnly]
    public static IntPtr EngineGlue_LoadKeyValues3(IntPtr pPath)
    {
        ...
        try
        {
            ...
            // Parse simple KeyValues format (text format with <key>value</key>)
            var kv3 = new SimpleKeyValues3();
    ...
    // Simple parser for KeyValues text format
    ...
    /// <summary>
    /// Gets a StringToken (unique identifier) for a string.
    /// Uses Sandbox.StringToken for compatibility with the existing system.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Gets the string corresponding to a StringToken.
    /// Allocates memory for the string and returns a pointer.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    public static void EngineGlue_AddSearchPath(IntPtr pPath, IntPtr pPathId, int nFlags)
    {
        ...
        lock (_searchPaths)
        {
            ...
            {
                    // Add to native file system if available
                try
                {
                    // Utiliser le système de fichiers managé
                    if (Directory.Exists(path))
                    {
                        // Zio file system already handles paths via BaseFileSystem
                        // We can also try to add to native system if available
                        if (_verboseLogging)
                        {
    ...
    /// <summary>
    /// Estimates process memory usage.
    /// </summary>
    [UnmanagedCallersOnly]
    public static ulong EngineGlue_ApproximateProcessMemoryUsage()
    {
        ...
        try
        {
            ...
            // On Linux, we could read /proc/self/status but it's complex
            // For now, return just managed memory
    ...
    public static IntPtr EngineGlue_ReadCompiledResourceFileJson(IntPtr pData)
    {
        ...
        try
        {
            ...
            // Source 2 compiled files can be binary or JSON
    ...
    /// <summary>
    /// Reads a block from a compiled resource file.
    /// Source 2 compiled resource files are binary files with named blocks.
    /// Signature: delegate* unmanaged&lt; IntPtr, IntPtr, int*, IntPtr &gt;
    /// Note: The first parameter is blockName (string), the second is the data buffer.
    /// </summary>
    [UnmanagedCallersOnly]
    public static IntPtr EngineGlue_ReadCompiledResourceFileBlock(IntPtr pBlockName, IntPtr pData, int* pSize)
    {
        ...
        try
        {
            ...
            // Source 2 compiled files are binary with a specific format
            // For now, we try to find the block in the file
            // Simplified format: search for the block by name in the data
            
            // If the requested block is "JSON" or "json", return the data as JSON
                ...
                blockName.Equals("json", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    *pSize = size;
                    // Return a pointer to the data (no copy for now)
                    return pData;
                }
            }
    ...
    /// <summary>
    /// Reads a compiled resource file as JSON from the file system.
    /// </summary>
    [UnmanagedCallersOnly]
    public static IntPtr EngineGlue_ReadCompiledResourceFileJsonFromFilesystem(IntPtr pPath)
    {
        ...
        try
        {
            ...
            if (File.Exists(path))
            {
                ...
                // Try to read as JSON (use inline logic)
    ...
    /// <summary>
    /// Requests a web authentication ticket (Steam).
    /// On Linux without Steam, this is a no-op.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Cancels a web authentication ticket request (Steam).
    /// On Linux without Steam, this is a no-op.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Gets the web authentication ticket (Steam).
    /// On Linux without Steam, returns IntPtr.Zero.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    // Counters for unique handles (same pattern as MaterialSystem)
    ...
    public static IntPtr Glue_Resources_GetMaterial(IntPtr pName)
    {
        ...
        // Use MaterialSystem to load/create the material
        // We call the native function directly via the material system
    ...
    public static IntPtr Glue_Resources_GetTexture(IntPtr pName)
    {
        ...
        // Use TextureSystem to load/create the texture
    ...
    public static IntPtr Glue_Resources_GetModel(IntPtr pName)
    {
        ...
        // Use ModelSystem to load/create the model
        // We call the function directly via the model system
    ...
    public static IntPtr Glue_Resources_GetAnimationGraph(IntPtr pName)
    {
        ...
        // Create an animation graph with Sandbox pattern (compatible with IAnimationGraph_*)
        // Same pattern as MaterialSystem: create a unique handle, not the BindingPtr
        ...
        // Create a unique handle for the animation graph (compatible with HAnimationGraph)
        // The handle must be unique and mapped to data (not BindingPtr directly)
    ...
    public static IntPtr Glue_Resources_GetShader(IntPtr pName)
    {
        ...
        // Create a shader with Sandbox pattern (compatible with CVfx_*)
        // Same pattern as MaterialSystem: create a unique handle, not the BindingPtr
    ...
    // Internal classes to store resource data
    // Pattern inspired by MaterialData in MaterialSystem.cs
    // Compatible with Sandbox system which uses ReferenceCount and BindingPtr
    // Note: TextureResourceData is now in Texture/TextureSystem.cs (TextureData)
    
    // Note: ModelResourceData is now in Model/ModelSystem.cs (ModelData)
    ...
    // ============================================
    // These functions are related to Steam Networking and can be disabled if Steam is not available.
    ...
    public static void Glue_Networking_RunCallbacks()
    {
        ...
        {
            // Steam disabled, nothing to do (no exception as this is called frequently)
            return;
        }
    ...
    public static void Glue_Networking_SetDebugFunction(int level, IntPtr func)
    {
        ...
        // If Steam is disabled, nothing to do (no-op)
    ...
    public static long Glue_Networking_GetAuthenticationStatus(IntPtr debugMsg)
    {
        ...
        // If Steam is disabled, return 0 (no exception)
    ...
    /// <summary>
    /// Creates a Steam Networking socket.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Creates an IP-based Steam Networking socket.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Gets a Steam Networking socket address.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Gets Steam Networking identity.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Destroys a Steam Networking poll group.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Gets messages from a Steam Networking poll group.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Connects to an IP address via Steam Networking.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Accepts a Steam Networking connection.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Sends a message via Steam Networking.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Gets the state of a Steam Networking connection.
    /// </summary>
    [UnmanagedCallersOnly]
    ...
    /// <summary>
    /// Gets the SteamID of a Steam Networking connection.
    /// </summary>
    [UnmanagedCallersOnly]
